...
function mapDbToUiProduct(p: ProductDB): UiProduct {
  ...
  const fabrics = Array.isArray(p.fabricTypes)
    ? p.fabricTypes
    : p.fabricTypes
    ? [p.fabricTypes]
    : [];
...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchRelatedProducts = async () => {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/midrange/products`, {
          headers: { "Content-Type": "application/json" },
        });
  ...
  const selectedVariant = useMemo(() => {
    ...
    const match = uiProduct.variants.find((v) => {
      ...
      const fabricMatch =
        !selectedFabric || v.attributes.fabric === selectedFabric;
    ...
    try {
      if (isWishlisted) {
        ...
        toast({
          ...
          description: `${uiProduct.name} removed.`,
        });
      } else {
        ...
        toast({
          ...
          description: `${uiProduct.name} added to wishlist.`,
        });
      }
    } catch (err) {
      toast({
  ...
  const handleShare = async () => {
    ...
    const text = `Check out ${uiProduct.name} on JSGALLOR! ${
      uiProduct.description?.slice(0, 100) || ""
    }`;
            ...
            <>
              <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
                ...
                <div className="space-y-6">
                  <div>
                    <p className="text-sm text-[#eef4df] mb-2">
                      {uiProduct.category}
                    </p>
                  ...
                  {availableColors.length > 0 && (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        {selectedColor ? (
                          <p className="text-xs text-[#d6dfbd]">
                          ...
                          return (
                            <button
                              ...
                              className={`relative h-10 rounded-lg border transition-all ${
                                selected
                                  ? "border-[#eef4df]"
                                  : "border-white/10 hover:border-[#d6dfbd]/40"
                              } ${hex ? "w-10" : "px-3"}`}
                            ...
                            >
                              {hex ? (
                                ...
                                <span className="text-sm text-[#f4f7ec]">
                                  {getColorName(c)}
                                </span>
                              )}
                  ...
                  {availableFabrics.length > 0 && (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        {selectedFabric ? (
                          <p className="text-xs text-[#d6dfbd] capitalize">
                            {selectedFabric}
                          </p>
                        ) : requireFabric ? (
                          <p className="text-xs text-red-300">Required</p>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="w-12 text-center font-semibold text-[#f4f7ec]">
                          {quantity}
                        </span>
                        ...
                        <button
                          ...
                          className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors text-[#eef4df] disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <div className="flex gap-3 flex-wrap">
                      <Button
                        ...
                        disabled={
                          !inStock || (uiProduct.hasVariants && !selectedVariant)
                        }
                      >
                  ...
                  <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/10">
                    {[
                      ...
                      <div key={feature.title} className="text-center">
                        ...
                        <p className="text-xs font-medium text-[#f4f7ec]">
                          {feature.title}
                        </p>
                  ...
                  {uiProduct.deliveryTime && uiProduct.deliveryTime.trim() && (
                    <div className="text-sm text-[#d6dfbd]">
                      Delivery Time:{" "}
                      <span className="font-medium text-[#f4f7ec]">
                        {uiProduct.deliveryTime}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="mt-16">
                <div className="flex items-center justify-between gap-4 mb-6">
                  <div>
                    <p className="text-sm text-[#d6dfbd] mb-1">
                      You may also like
                    </p>

                    <h2 className="text-2xl md:text-3xl font-bold text-[#f4f7ec]">
                  ...
                  <Link
                    to="/products"
                    className="hidden md:flex items-center gap-2 text-sm text-[#eef4df] hover:underline"
                  >
                    View All
                ...
                {relatedLoading ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[...Array(4)].map((_, index) => (
                      <div
                        key={index}
                        className="bg-[#4b5e29] rounded-xl overflow-hidden border border-white/10"
                      >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {relatedProducts.map((item) => {
                      const priceData = computeFinalPrice(
                        item.basePrice,
                      ...
                      return (
                        <Link
                          key={item._id}
                          to={`/product/${item._id}`}
                          className="group bg-[#4b5e29] rounded-xl overflow-hidden border border-white/10 hover:border-[#eef4df]/40 transition-all"
                        >
                          ...
                          <div className="p-4">
                            <p className="text-xs text-[#d6dfbd] mb-1">
                              {item.category}
                            </p>

                            <h3 className="text-sm font-semibold text-[#f4f7ec] line-clamp-2 min-h-[40px]">
                              {item.name}
                            </h3>

                            <div className="flex items-center gap-2 mt-3 flex-wrap">
                              {item.discountPercent > 0 && (
                                <span className="text-xs text-[#d6dfbd] line-through">
                                  {formatINR(priceData.originalPrice)}
                                </span>
                              )}

                              <span className="text-lg font-bold text-[#eef4df]">
                                {formatINR(priceData.finalPrice)}
                              </span>
                            </div>
                            ...
                            <div className="mt-4 flex items-center justify-between text-xs text-[#d6dfbd]">
                              <span>
                                {item.totalStock > 0 ? "In Stock" : "Out of Stock"}
                              </span>

                              <span className="inline-flex items-center gap-1 text-[#eef4df]">
                                View
                  ...
                  <div className="rounded-xl border border-white/10 bg-[#4b5e29] p-6 text-center">
